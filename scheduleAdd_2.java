...
public class scheduleAdd_2 extends JFrame implements ActionListener{
		...
		private boolean todayMode = false;
		boolean selectCheck = true;
		private String userid = "";
		int idx;
		public scheduleAdd_2(String userid, String date) {		//today plan용도
			this.userid = userid;
			this.todayDate = date;
			todayMode = true;
			setTitle("일정추가");		
		...
		public scheduleAdd_2(String userid) {		//today plan용도
		...
		if(obj==save_btn) {
			...
			if(continueInput==true) {
				...
				if(!todayMode)
					...
					new calendarPlanlistView(userid, todayDate);
				dispose();
				selectCheck = true;	
			}
			}
		}
		else if(obj==getback_btn) {
			if(!todayMode)
				new scheduleAct(userid);
			else if(todayMode)
				new calendarPlanlistView(userid, todayDate);
